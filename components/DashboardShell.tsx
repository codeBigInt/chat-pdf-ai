...
export function DashboardShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <SidebarProvider defaultOpen={isSidebarOpen} onOpenChange={setIsSidebarOpen}>
      <div className="flex h-scree bg-black w-full overflow-hidden">
        <AppSidebar />
        <div className="flex-1 w-full h-screen relative overflow-auto">
          <DashboardNav />
          <SidebarInset>
            <main className="p-6 relative bg-black min-h-screen w-full h-full">
              {children}
              <Dialog>
                <DialogTrigger asChild>
                  <button className="fixed z-30 rounded-full shadow-lg bg-gradient-to-r from-purple-500 to-pink-500  text-white flex items-center justify-center p-3 bottom-8 right-8 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 hover:outline-none hover:ring-2 hover:ring-blue-500 hover:ring-offset-2">
                    <Plus size={24} />
                  </button>
                </DialogTrigger>
